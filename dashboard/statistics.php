...
<head>
    ...
    <title>OpenAir Booking | Statistics</title>
        ...
        <div class="container">
            ...
            <div class="row my-5">
                <div class="col-sm-4 buttonlist">
                    <button id="view" class="btn crud-btn my-1" onclick="showHideCRUD(this.id)" type="back">Current</button>
                        ...
                        <div class="table-responsive-xl">
							...
							<?php
                                ...
                                while ($record = mysqli_fetch_assoc($resultset)) {
                                    echo '<td>' . $record['total_customers'] . '</td>';
                                }

                                $sql = "SELECT COUNT(*) AS total_staff FROM staff;";
                                $resultset = mysqli_query($conn, $sql) or die("database error:" . mysqli_error($conn));
                                while ($record = mysqli_fetch_assoc($resultset)) {
                                    echo '<td>' . $record['total_staff'] . '</td>';
                                }

                                $sql = "SELECT COUNT(*) AS total_sales FROM payments;";
                                $resultset = mysqli_query($conn, $sql) or die("database error:" . mysqli_error($conn));
                                while ($record = mysqli_fetch_assoc($resultset)) {
                                    echo '<td>' . $record['total_sales'] . '</td>';
                                }

                                $sql = "SELECT COUNT(*) AS total_branches FROM branch;";
                                $resultset = mysqli_query($conn, $sql) or die("database error:" . mysqli_error($conn));
                                while ($record = mysqli_fetch_assoc($resultset)) {
                                    echo '<td>' . $record['total_branches'] . '</td>';
                                }

                                $sql = "SELECT SUM(pay_amount) AS total_income FROM payments;";
                                $resultset = mysqli_query($conn, $sql) or die("database error:" . mysqli_error($conn));
                                while ($record = mysqli_fetch_assoc($resultset)) {
                                    echo '<td>' . $record['total_income'] . '</td>';
                                }

                                $sql = "SELECT COUNT(*) AS taxes_paid FROM package;";
                                $resultset = mysqli_query($conn, $sql) or die("database error:" . mysqli_error($conn));
                                while ($record = mysqli_fetch_assoc($resultset)) {
                                    echo '<td>' . $record['taxes_paid'] . '</td>';
                                }

                                $sql = "SELECT COUNT(*) AS wages_paid FROM package;";
                                $resultset = mysqli_query($conn, $sql) or die("database error:" . mysqli_error($conn));
                                while ($record = mysqli_fetch_assoc($resultset)) {
                                    echo '<td>' . $record['wages_paid'] . '</td>';
                                }

                                $sql = "SELECT COUNT(*) AS profit FROM package;";
                                $resultset = mysqli_query($conn, $sql) or die("database error:" . mysqli_error($conn));
                                while ($record = mysqli_fetch_assoc($resultset)) {
                                    echo '<td>' . $record['total_staff'] . '</td>';
                                }
